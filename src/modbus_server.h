...
class ModbusServer : public uart::UARTDevice, public Component {
 public:
  ...
  struct ResponseRead
  {
    ...
    {
      // MSB first
      auto bigEndianValue = convert_big_endian(value);
  ...
  // using on_receive_cb = std::function<void(uint8_t function_code, const uint8_t* data, size_t data_size)>;
  using on_receive_read = std::function<ResponseRead(uint8_t function_code, const RequestRead& request)>;

  ModbusServer(uint8_t address, on_receive_read on_receive)
  : address_(address)
  , on_receive_read_(on_receive)
  {}
  ...
  void loop() override
  {
    // loop is called every ~16ms, so we can not rely on timing (3.5 chars between frames see https://en.wikipedia.org/wiki/Modbus)
    // instead parse the rx_buffer for valid frames(address, function-code, length, crc).
    // Read all from uart
...
//        ESP_LOGV(TAG, "Modbus received Byte  %d (0X%x)", byte, byte);
 ...
 protected:
  uint8_t address_;
  on_receive_read on_receive_read_;
  ...
  uint32_t parse_modbus_frame() 
  {
    ...
    uint16_t remote_crc = static_cast<uint16_t>(*(begin + frame_size - 2)) | (static_cast<uint16_t>(*(begin + frame_size - 1)) << 8);
    if (computed_crc != remote_crc)
    {
      ESP_LOGD(TAG, "Invalid CRC");
      // std::cout << std::hex << "invalid crc, computed_crc.lo = 0x" << (computed_crc & 0xFF) << ", computed_crc.hi = 0x" << (computed_crc >> 8) << std::dec << std::endl;
      return 1; // remove 1 byte
    }
    ...
    if (address_ == address)
    {
      ...
      ResponseRead response = on_receive_read_(function_code, request);
